...
bool InputGeom::loadMesh(rcContext* ctx, const std::string& filepath)
{
    ...
    {
        ctx->log(RC_LOG_PROGRESS, "buildTiledNavigation: Successfully loaded '%s'", filepath.c_str());
    }
    std::cout << "Vertices count: " << m_mesh->getVertCount() << std::endl;
    std::cout << "Triangles count: " << m_mesh->getTriCount() << std::endl;

	rcCalcBounds(m_mesh->getVerts(), m_mesh->getVertCount(), m_meshBMin, m_meshBMax);

    std::cout << "BoundsMin: " << m_meshBMin[0] << ", " << m_meshBMin[1] << ", " << m_meshBMin[2] << std::endl;
    std::cout << "BoundsMax: " << m_meshBMax[0] << ", " << m_meshBMax[1] << ", " << m_meshBMax[2] << std::endl;
    ...
    std::cout << "buildTiledNavigation: Chunky mesh build completed!" << std::endl;
    return true;
}
...
bool InputGeom::load(rcContext* ctx, const std::string& filepath)
{
    ...
    if (extension == ".obj")
        std::cout << ".obj file loaded: " << filepath << std::endl;
