...
void EQOA_TileMesh::handleSettings()
{
...
dtNavMesh* EQOA_TileMesh::handleBuild()
{
...
void EQOA_TileMesh::collectSettings(BuildSettings& settings)
{
	Sample::collectSettings(settings);
...
void EQOA_TileMesh::getTilePos(const float* pos, int& tx, int& ty)
{
	if (!m_geom) return;
...
void EQOA_TileMesh::buildAllTiles()
{
	if (!m_geom) return;
	if (!m_navMesh) return;
...
unsigned char* EQOA_TileMesh::buildTileMesh(const int tx, const int ty, const float* bmin, const float* bmax, int& dataSize)
{
